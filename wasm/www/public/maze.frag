...
void main() {
  ...
  bvec2 in_wall = lessThan(mod(normalized_positions, cell_size), vec2(u_wall_size, u_wall_size));
  ...
  outColour = vec4(any(in_wall) ? 1.0 : v_pos.x, any(in_wall) ? 1.0 : v_pos.y, 1.0, 1);
}
